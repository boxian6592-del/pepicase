    ...
    if (data && data.length > 0) {
        data.forEach(purchase => {
            ...
            if (!document.getElementsByClassName(`purchases-container-id-${purchase.id}`).length) {
                ...
                const block = createItemBlock(purchase.id, purchase.name_product, purchase.price, purchase.image, purchase.quantity);
...
function createPurchaseBlock(id, order_date) {
    ...
    block.className = `purchases-container-id-${id}`;
    ...
    orderDateDiv.className = `order-id-${id}`;
