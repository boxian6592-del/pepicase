<?php include(APPPATH.'views/components/usual-links.php'); ?>
<?php include(APPPATH.'views/components/top-header.php'); ?>

        <div class="lexend d-flex flex-column justify-content-center align-items-center" style ="min-height:50vh;">
            <h1 style ="font-size: 50px">A confirmation email has been sent to your email!</h1>
            <h2 style ="font-size: 50px">Please check your email accordingly.</h2>
            <a href="/pepicase/public/" style ="color:black;">Go back to the home page here</a>
        </div>

<?php include(APPPATH.'views/components/bottom-footer.php'); ?>